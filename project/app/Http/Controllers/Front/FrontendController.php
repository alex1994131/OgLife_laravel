...
// -------------------------------- HOME PAGE SECTION ----------------------------------------
    ...
    public function index(Request $request)
    {
        ...
        return view('front.index', compact('ps', 'sliders', 'top_small_banners', 'genetics_products', 'main_slider_products', 'colorsetting_style1', 'colorsetting_style2'));
    }
...
// -------------------------------- STRAIN SECTION ----------------------------------------
    public function strain(Request $request) {
        $this->code_image();
        $strains = Strain::orderBy('created_at', 'desc')->paginate(6);
        if ($request->ajax()) {
            return view('front.pagination.strain', compact('strains'));
        }
        $search = '';
        return view('front.strain', compact('strains', 'search'));
    }

    public function strainsearch(Request $request)
    {
        $this->code_image();
        $search = $request->search;
        if($search == '') {
            ...
            $strains = Strain::where('name', 'like', '%' . $search . '%')->orWhere('description', 'like', '%' . $search . '%')->orderBy('created_at', 'desc')->paginate(6);
        }
    ...
    public function strainshow($id) {
        $this->code_image();
        $strain = Strain::find($id);
        return view('front.strainshow', compact('strain'));
    }

    public function strainadd() {
        $this->code_image();
        return view('front.strainadd');
    }

    public function strainsave(Request $request) {
        $rules =
        [
            'name' => 'required',
            'effect' => 'required',
        ...
        try{
            ...
            if(!empty($strain)) {
                if($request->hasfile('gallery'))
                {
                    foreach($request->file('gallery') as $file)
                    {
                        ...
                        $origin_name = str_replace(array( '\'', '"', ',' , ';', '<', '>', '!', '@', '#', '$', '%', '^', '&', '*', ':' ), '', $origin_name);
                        $name = time(). $origin_name .'.'.$file->extension();
                        $file->move(public_path().'/assets/images/strainTemp/', $name);

                        $gallery = new StrainGalleryTemp;
                        $gallery->strain_temp_id = $strain->id;
                        $gallery->path = 'assets/images/strainTemp/'. $name;
                        $gallery->save();
                    }
                }
            }
            ...
            return redirect()->route('front.strain');
        }
        catch (\Throwable $th) {
            return redirect()->back();
        }
